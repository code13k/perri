...
import org.code13k.perri.model.config.channel.HangoutInfo;
import org.slf4j.Logger;
import org.slf4j.LoggerFactory;

import java.util.function.Consumer;

public class HangoutSender extends BasicSender {
    // Logger
    private static final Logger mLogger = LoggerFactory.getLogger(HangoutSender.class);
    ...
    public void send(MessageOperation messageOperation, Consumer<Integer> consumer) {
        ...
        try {
            // Get request uri
            HangoutInfo hangoutInfo = (HangoutInfo)messageOperation.getChannelInfo();
            String requestUri = hangoutInfo.getIncomingWebhookUrl();
    ...
    private JsonObject makeBody(MessageOperation messageOperation) {
        try {
            ...
            // End
            mLogger.debug("HangoutSender Body : " + vertxJsonObject.toString());
            return vertxJsonObject;
        } catch (Exception e) {
            mLogger.error("Failed to make hangout body", e);
        }
        return null;
    }
}